<template>
  <div class="w-full flex flex-col h-full">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item
                ...
                :disabled="! currentLessonId"
              >
                Validate OrderIndex (Bài này)
              </el-dropdown-item>
              <el-dropdown-item
                command="validate-all"
                :icon="Setting"
              >
                Validate OrderIndex (Tất cả)
              </el-dropdown-item>
              <el-dropdown-item
    ...
    <el-empty
      ...
      description="Vui lòng chọn bài đọc để quản lý câu hỏi"
      :image-size="120"
    />
    <el-empty
      v-else-if="!questions || questions.length === 0"
      description="Bài đọc này chưa có câu hỏi nào"
      :image-size="120"
    >
...
const handleDropdownCommand = async (cmd) => {
  if (cmd === 'validate-lesson') {
    // Validate questions của bài đọc hiện tại
    try {
      await store.validateQuestionsOrder(currentLessonId.value)
      await loadQuestions()
    } catch (error) {
      console.error('Validate failed:', error)
    }
  } else if (cmd === 'validate-all') {
    // Validate tất cả questions của module Reading
    try {
      await store.validateAllQuestionsOrder()
    } catch (error) {
      console.error('Validate all failed:', error)
    }
  } else if (cmd === 'health-check') {
    // Health check toàn bộ module Reading
    try {
