<template>
  <div class="w-full">
    <div v-if="isTrueFalseMode" class="mb-4 bg-blue-50 dark:bg-blue-900/10 p-4 rounded-lg border border-blue-100 dark:border-blue-800">
      <div class="text-sm font-bold text-blue-800 dark:text-blue-300 mb-3">Đáp án đúng là:</div>
      <el-radio-group v-model="trueFalseValue" @change="emitTrueFalseUpdate" class="!flex !flex-col !items-start gap-3">
        <el-radio :label="true" border class="!ml-0 !w-full !mr-0 !bg-white dark:!bg-[#2c2c2c] !border-green-300">
          <span class="text-green-600 font-bold">TRUE (Đúng)</span>
        </el-radio>
        <el-radio :label="false" border class="!ml-0 !w-full !bg-white dark:!bg-[#2c2c2c] !border-red-300">
    ...
    <div v-else class="space-y-3">
      <div v-for="(opt, index) in localMetadata.options" :key="index" class="flex items-center gap-3 bg-white dark:bg-[#2c2c2c] p-2 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm group">
        <div class="flex items-center justify-center w-10 shrink-0">
           <el-radio v-model="correctAnswerIndex" :label="index" @change="handleCorrectChange" size="large">
             <span class="hidden">.</span> </el-radio>
        </div>

        <div class="w-8 h-8 rounded bg-gray-100 dark:bg-gray-700 flex items-center justify-center font-bold text-gray-500 dark:text-gray-300 text-sm shrink-0">
           {{ String.fromCharCode(65 + index) }}
        </div>

        <el-input v-model="opt.text" placeholder="Nhập nội dung đáp án..." class="flex-1" @input="emitUpdate" />

        <el-button
          type="danger" circle plain text
...
const initData = () => {
  ...
  if (isTrueFalseMode.value) {
    // Xử lý đầy đủ các trường hợp khởi tạo TRUE/FALSE
    let needsEmit = false

    if (meta.options?.length) {
      const trueOpt = meta.options.find(o => o.text === 'True')
      if (trueOpt) {
        trueFalseValue.value = trueOpt.isCorrect
      }
      // Nếu có options nhưng thiếu correctAnswer -> cần emit
      ...
      // Trường hợp tạo mới: set mặc định TRUE và emit
      trueFalseValue.value = true
      needsEmit = true
    }

    // Chỉ emit nếu thực sự cần (tạo mới hoặc thiếu correctAnswer)
    if (needsEmit) {
      nextTick(() => {
        emitTrueFalseUpdate()
      })
...
const emitTrueFalseUpdate = () => {
  ...
  // ✅ FIX: Emit cả options VÀ correctAnswer để validate pass
