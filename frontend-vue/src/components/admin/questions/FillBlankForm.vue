<template>
  <div class="w-full">
    <div v-if="questionType === 'FILL_BLANK'" class="mb-4 p-3 bg-gray-50 dark:bg-[#2c2c2c] rounded-lg border border-gray-200 dark:border-gray-700 flex justify-between items-center">
        <span class="text-sm font-bold text-gray-700 dark:text-gray-300">Ngân hàng từ (Word Bank)</span>
    ...
    <div v-if="hasWordBank && questionType === 'FILL_BLANK'" class="mb-4">
        <el-select
            ...
            placeholder="Nhập các từ gây nhiễu..."
            class="!w-full"
            @change="emitUpdate"
        />
    </div>
    ...
    <div class="space-y-3">
      <div v-for="(blank, index) in localMetadata.blanks" :key="index" class="bg-white dark:bg-[#2c2c2c] p-3 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm relative group">
         <div class="flex justify-between items-center mb-2">
            <el-tag effect="dark" size="small" type="success" class="!rounded">Vị trí #{{ blank.position }}</el-tag>
            <el-button type="danger" link size="small" @click="removeBlank(index)" :disabled="localMetadata.blanks.length <= 1">Xóa</el-button>
         </div>
...
const addBlank = () => { localMetadata.value.blanks.push({ position: localMetadata.value.blanks.length + 1, correctAnswers: [] }); emitUpdate() }
const removeBlank = (idx) => { localMetadata.value.blanks.splice(idx, 1); emitUpdate() }
const emitUpdate = () => {
  // Logic emit tương tự file cũ
  const payload = { blanks: localMetadata.value.blanks, wordBank: hasWordBank.value ? localMetadata.value.wordBank : [] }
