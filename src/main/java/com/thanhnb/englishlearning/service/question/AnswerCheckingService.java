...
public class AnswerCheckingService {
    ...
    private boolean checkMultipleChoice(Question question, SubmitAnswerRequest answerRequest) {
        ...
        log.debug("Multiple choice check: questionId={}, selectedOptionId={}, isCorrect={}", 
    ...
    private boolean checkFillBlank(Question question, SubmitAnswerRequest answerRequest) {
        ...
        if (!isCorrect) {
            log.debug("Fill blank incorrect - User: '{}', Expected one of: {}", 
                    userAnswer, 
                    Arrays.toString(Arrays.stream(correctAnswers)
    ...
    public static class KeywordPreviewDTO {
        ...
        public List<String> getVariantDetails() { return variantDetails; }
        public String getCheckingLogic() { return checkingLogic; }
    }
     ...
     *    User: "were" → ERROR_WRONG_SUBJECT_VERB (chủ ngữ "I" cần "was" không phải "were")
     ...
     *    User: "Me like learning English" → ERROR_WRONG_PRONOUN (dùng "me" thay vì "I")
     */
    ...
    private ErrorAnalysisResult analyzeTranslationError(Question question, String userAnswer, String correct) {
        ...
        if (userAnswer.length() >= 10) {
            ...
            if (words.length >= 3) {
                ...
                boolean hasVerb = Arrays.stream(words).anyMatch(w -> 
                    w.matches(".*(am|is|are|was|were|be|have|has|had|do|does|did|like|love|go|come|make|take|get).*")
                );
 ...
 * SubmitAnswerRequest request = new SubmitAnswerRequest(questionId, "was", null);
 * 
 ...
 * 
 * Question translationQ = questionRepository.findById(questionId).orElseThrow();
 * String userAnswer = "I like learning English";
 * 
 * TranslationCheckResult result = answerCheckingService.checkTranslationWithScore(
 ...
 * 
 * SubmitAnswerRequest wrongRequest = new SubmitAnswerRequest(questionId, "am", null);
 ...
 * // Output: Hệ thống check: exact match HOẶC có >= 75% keywords HOẶC similarity >= 85%
 * 
 * 
